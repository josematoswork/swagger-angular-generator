...
export function processMethod(
  ...
  let methodDef = "";
  let interfaceDef = "";
  const url = method.url.replace(
    /{([^}]+)}/g,
    (_, key) => `\${${getAccessor(key, "pathParams")}}`
  );
  ...
  methodDef += makeComment(
    [method.summary, method.description, method.swaggerUrl].filter(Boolean)
  );
  ...
  const body =
    `return this.http.${method.methodName}<${method.responseDef.type}>` +
    `(environment.apiUrl + \`${method.basePath}${url}\`${params});`;
 ...
 * @param paramGroups
 */
function getParamSeparation(
  paramGroups: Partial<Record<ParamLocation, Parameter[]>>
): string[] {
