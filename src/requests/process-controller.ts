...
export function processController(
  ...
  methods.forEach((controller) => {
    controller.operationId =
      ...
      controller.method.parameters.filter(
        (a) => a.name.toLowerCase() !== "body"
      ).length > 0
        ? "By" +
          controller.method.parameters
            .filter((a) => a.name.toLowerCase() !== "body")
            .map((a) => _.upperFirst(a.name))
            .join("")
        : "");
    if (simpleNames.filter((n) => n === controller.simpleName).length > 1) {
      const preserveCapitals = controller.operationId
        ? controller.operationId.replace(/([A-Z])/g, "-$1")
        : "";
