...
export function writeFile(
  ...
  if (fileType === "ts") {
    ...
    //   disable = `/* eslint:disable:${disableFlags.join(" ")} */\n`;
    if (header) header += "\n";
...
export function merge(
  ...
  const othersFiltered = others.filter((elem) => {
    return !favoured.find((subElem) =>
      keys.map((k: string) => elem[k] === subElem[k]).every(Boolean)
    );
  });
